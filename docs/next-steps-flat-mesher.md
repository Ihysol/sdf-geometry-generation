...
- `VoxelGrid` implements `IFlatDenseVolumeData`.
- `DualContouringFlatOctreeMesher` consumes `IFlatAdaptiveVolumeData`.
- `FlatOctreeLayout` stores `NodeSizeInCells` and caches runtime lookup data:
  - surface leaf indices
  - exact leaf coordinate map
  - subtree sizes
  - child index by octant
- Flat chunk meshing now filters surface leaves against owned grid bounds before edge processing.
- Mesher paths still mostly use tree traversal (`OctreeNode`) in runtime.
- `SparseVoxelOctree` now has its own sampler/builder/volume path and still adapts to `OctreeVolume` for tree-based meshers.

## Recommended Implementation Order

1. Migrate remaining meshers to flat input (no behavior changes first)
...
2. Switch runtime to flat-first
...
5. Parallelization (after flat is stable)
