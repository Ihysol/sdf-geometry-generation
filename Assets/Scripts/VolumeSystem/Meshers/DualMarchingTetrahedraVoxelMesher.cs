...
/// </summary>
public class DualMarchingTetrahedraVoxelMesher : IVolumeMesher<VoxelGrid>
{
    ...
    public void BuildMesh(VoxelGrid volume, float isoLevel, Mesh targetMesh)
    {
        ...
        int resolutionX = Mathf.Max(0, volume.GridSize.x - 1);
        int resolutionY = Mathf.Max(0, volume.GridSize.y - 1);
        int resolutionZ = Mathf.Max(0, volume.GridSize.z - 1);
        ...
        for (int x = 0; x < resolutionX; x++)
        {
            ...
            if (!IsOwnedCell(c, origin, cell))
                continue;
            PolygonizeCell(c, origin, cell, volume, isoLevel);
        }
    ...
    private void PolygonizeCell(Vector3Int cellCoord, Vector3 origin, Vector3 cell, VoxelGrid volume, float iso)
    {
    ...
    private float SampleCorner(VoxelGrid volume, Vector3Int gc)
    {
        if (_cornerCache.TryGetValue(gc, out float cached))
            return cached;
        float value = volume.GetValue(gc.x, gc.y, gc.z);
